...
function PublicParameters() {
    ...
    let lastGcreated, lastHcreated=0
    constructor() 
    // public
     {
    ...
    function createGVector()
    //  public returns (bool success) 
     {
        require(lastGcreated < m);
        // uint256 
        let maxToCreate = m - lastGcreated;
        if (maxToCreate > n) {
            maxToCreate = n;
        }
        // uint256[2] memory reusablePoints;
        let reusablePoints=[2]
        for ( i = lastGcreated; i < lastGcreated+maxToCreate; i++) {
    ...
    function createHVector() 
    // public returns (bool success) 
    {
        require(lastHcreated < m);
        // uint256 
        let maxToCreate = m - lastHcreated;
        if (maxToCreate > n) {
            maxToCreate = n;
        }
        // uint256[2] memory reusablePoints;
        reusablePoints=[2];
        for ( i = lastHcreated; i < lastHcreated+maxToCreate; i++) {
    ...
    function signaturePublicGenerator() 
    // public pure returns (uint256[2] point) 
    {
        return [uint256(1), uint256(2)];
    }

    function peddersenBaseG() 
    // public view returns (uint256[2] point) 
    {
        // bytes32 
        let h = keccak256("G");
    ...
    function getGVectorComponent( index) 
    // public view returns (uint256[2] point) 
    {
        require(index < m);
        // bytes32
        let  h = keccak256("G", index.uintToBytes());
        alt_bn128.G1Point memory g1p = alt_bn128.uintToCurvePoint(uint256(h));
        return [g1p.X, g1p.Y];
    }

    function getHVectorComponent(
        // uint256
         index) 
        //  public view returns (uint256[2] point) 
         {
        require(index < m);
        // bytes32 
        let h = keccak256("H", index.uintToBytes());
        alt_bn128.G1Point memory g1p = alt_bn128.uintToCurvePoint(uint256(h));
        return [g1p.X, g1p.Y];
    }
    
    function getGVector() 
    // public view returns (uint256[2*m] points) 
    {
        return gVector;
    }
    ...
    function generator()
    //  public view returns (alt_bn128.G1Point memory p) 
     {
    ...
    function gs() 
    // public view returns(alt_bn128.G1Point[m] memory points)
    {
    ...
    function assemblePointsFromEncodings(
        // uint256[m*2] 
        _pointsEncoding) 
    // internal pure returns(alt_bn128.G1Point[m] memory points) 
    {
        for ( i = 0; i < m; i++) {
    ...
    // (uint256[2]
         _encoding)
        //   internal pure returns(alt_bn128.G1Point memory point)
           {
