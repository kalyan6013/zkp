...
// contract MultiRangeProofVerifier {
    ...
    // returns (bool) 
    {
        ...
        let numOfCommitments = commitments.length/2;
        ...
        let yPrecomputed = uint256(keccak256(commitments, multiRangeProof.aI.X, multiRangeProof.aI.Y, multiRangeProof.s.X, multiRangeProof.s.Y)).mod();
    ...
    struct Board {
        ...
        let twos=[];
        // uint256[m] 
        let twoTimesZSquared=[m];
        // uint256 zSum;
        // uint256 x;
        let zSum,x,k,uChallenge,bitsPerNumber
    ...
    // (alt_bn128.G1Point[m] ps, uint256[m] ss)
    ...
    function sumScalars(ys=[])
    // (uint256[] ys) internal pure returns (uint256 result) 
    {
    ...
    function sumPoints(ps=[])
    // (alt_bn128.G1Point[m] ps) internal view returns (alt_bn128.G1Point sum) 
    {
        sum = ps[0];
        for ( i = 1; i < m; i++) {
            sum = sum.add(ps[i]);
    ...
    function commit(ps=[],ss=[])
    // (alt_bn128.G1Point[m] ps, uint256[m] ss) internal view returns (alt_bn128.G1Point commit) 
    {
        for ( i = 1; i < m; i++) {
    ...
    // (uint256 base) internal pure returns (uint256[m] powers) 
    {
        powers[0] = 1;
        powers[1] = base;
        for (
    ...
    // (uint256 base, uint256 firstPower, uint256 numberOfPowers) internal pure returns (uint256[] powers) 
    {
    ...
    function times(v=[m],x)
    // (uint256[m] v, uint256 x) internal pure returns (uint256[m] result) 
    {
        for ( i = 0; i < m; i++) {
            result[i] = v[i].mul(x);
        }
    ...
    function timesAndConcat(v=[],xs)
    // (uint256[] v, uint256[] xs) internal pure returns(uint256[m] result) 
    { //twos, zs. twos.times(z).reduce()
        // uint256
