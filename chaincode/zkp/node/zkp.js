...
let Chaincode = class {
  ...
  async initData(stub, args, thisClass) {
    ...
    let vehicleState = await stub.getState(vehicleNo);
    let driverState = await stub.getState(drivingLicenceNo);
    ...
    await stub.putState(vehicleNo, Buffer.from(JSON.stringify(car)));
...
//    console.log('Blockchain data : ' + await stub.getState(vehicleNo));
    let vehicleNoAsbytes = await stub.getState(vehicleNo);
    console.log('This is vehicle Data : '+ vehicleNoAsbytes);
   let drivingLicenceNooAsbytes = await stub.getState(drivingLicenceNo);
